...
const AddressTab = (props: { clientData: Array<Object> }) => {
    ...
    const postalAddressColumn = [{ label: 'ID', property: 'postal_address:display_id' }];
    ...
    const populateAddressData = (data: any) => {
        ...
        addressList.push({
            url: getLink(data, 'person:postal_address_list')
                ? getLink(data, 'person:postal_address_list')
                : getLink(data, 'organization:postal_address_list'),
            id: 'postal-address',
        });
        addressList.push({
            url: getLink(data, 'person:bank_account_list')
                ? getLink(data, 'person:bank_account_list')
                : getLink(data, 'organization:bank_account_list'),
            id: 'bank-account',
        });
        addressList.push({
            url: getLink(data, 'person:phone_address_list')
                ? getLink(data, 'person:phone_address_list')
                : getLink(data, 'organization:phone_address_list'),
            id: 'telecom-address',
        });
        addressList.push({
            url: getLink(data, 'person:e_mail_address_list')
                ? getLink(data, 'person:e_mail_address_list')
                : getLink(data, 'organization:e_mail_address_list'),
    ...
    return (
        <>
            <section className="pt-3">
                <div className="pb-4">{renderTable('_POSTAL_ADDRESS', postalAddressColumn, 'postal-address')}</div>
                <div className="pb-4">{renderTable('_BANK_ACCOUNTS', bankAccountColumns, 'bank-account')}</div>
                <div className="pb-4">{renderTable('_PHONE_ADDRESS', phoneAddressColumns, 'telecom-address')}</div>
                <div className="pb-4">{renderTable('_EMAIL_ADDRESS', emailAddressColumns, 'electronic-address')}</div>
            </section>
        </>
    );
};
