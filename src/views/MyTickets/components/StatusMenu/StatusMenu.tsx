...
import { PanelTickets } from '../../StyledTickets';
...
const StatusMenu = (props: any) => {
    const {title, items, onItemClick, ticketTitle, countArray = {} } = props;
    ...
    return (
        <PanelTickets className="tickets-menu">
            ...
            <PanelTickets.ParentList className="mt-2 pr-1 pl-2">
                {/* <h5>Views</h5> */}
                {items && items.map((item: any) => <PanelTickets.List key={item.status} className={`cursor-pointer list-group-item d-flex justify-content-between align-items-center border-0 rounded-0 ${selected(item.status) ? 'active' : ''}`} onClick={() => onItemClick()}>
